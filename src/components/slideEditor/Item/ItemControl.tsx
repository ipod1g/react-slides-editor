...
const ItemControl = React.memo(({ itemId }: { itemId: string }) => {
  ...
  return (
    <div
      ...
      className="border-accent-light rounded-md border-2 w-[250px] min-w-[120px] h-10 absolute bg-primary-white -top-12 right-0 group-hover/resize:opacity-100 transition-opacity opacity-0 flex scale-0 group-hover/resize:scale-100 delay-200"
    >
      <div
        role="button"
        tabIndex={0}
      ...
      <div
        ...
        className="border-accent-mid/50 text-xl px-2 py-1 flex-shrink-0 cursor-pointer"
      >
        ❌
      </div>
    </div>
