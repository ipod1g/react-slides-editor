// @ts-expect-error immer does not provide @types
...
const useSlidesStore = create<StoreModel>()(
  devtools(
    // persist(
    (set, get) => ({
      slides: [],
      ...
      actions: {
        setSlides: (slideArray: Slide[]) => {
          set(
            produce((state) => {
              state.slides.slides = slideArray;
            })
          );
        },
        ...
        updateItemPosition: (slideIndex, _type, itemId, position) =>
          set(
            produce((state) => {
              const slide = state.slides.slides[slideIndex];
